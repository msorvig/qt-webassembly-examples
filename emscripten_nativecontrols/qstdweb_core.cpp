...
void FileReader::onLoad(const std::function<void (const emscripten::val&)> &onLoad)
{
    m_onLoad.reset(new EventCallback(m_fileReader, "load", onLoad));
}

void FileReader::onError(const std::function<void (const emscripten::val&)> &onError)
{
    m_onError.reset(new EventCallback(m_fileReader, "error", onError));
}

void FileReader::onAbort(const std::function<void (const emscripten::val&)> &onAbort)
{
...
EventCallback::EventCallback(const emscripten::val &element, const std::string &name, const std::function<void (const emscripten::val&)> &fn)
   :m_element(element)
   ,m_name(name)
   ,m_fn(fn)
{
    m_element.set(contextPropertyName(m_name).c_str(), emscripten::val(intptr_t(this)));
    m_element.set((std::string("on") + m_name).c_str(), emscripten::val::module_property("qtStdWebEventCallbackActivate"));
}

EventCallback::~EventCallback()
{
    m_element.set((std::string("on") + m_name).c_str(), emscripten::val::undefined());
}

void EventCallback::activate(const emscripten::val& event)
{
    emscripten::val target = event["target"];
    std::string eventName = event["type"].as<std::string>();
    EventCallback *that =
        reinterpret_cast<EventCallback *>(target[contextPropertyName(eventName).c_str()].as<intptr_t>());
    that->m_fn(event);
}
